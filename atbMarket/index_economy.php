...
$endDateTag = $endDatePage->find('.date')[0];
...
/* Parsed Main Page */
$pageText = getPageContentAsString('https://www.atbmarket.com/hot/akcii/economy/');
...
$productSection = $pageDOM->find('.list_wrapper');
if ($productSection != null) {
	$items = $productSection[0]->find('li');
	if ($items != null) {
		foreach ($items as $item) {
			...
			if ($moreNameBlock != null) {
				...
				$newMoreName = '';
				for ($i = 0; $i < count($fullMoreNameArray)-1; $i++) {
