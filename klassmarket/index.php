...
/* Parsed Main Page */
$GLOBALS['baseURL'] = 'https://klassmarket.ua/aktsii/katalog-aktsionnykh-tovarov';

/* Products */
$GLOBALS['all_items'] = [];
function parseThisPage($url = 'https://klassmarket.ua/aktsii/katalog-aktsionnykh-tovarov')
{
	$pageText = getPageContentAsString($url);
	$page = str_get_html($pageText);
	$pageContent = $page->find('.grid-start')[0];
	...
	$items = $pageContent->find('.product-sale');
	if ($items != null) {
		foreach ($items as $item) {
			$itemObject = [];

			/* Product Image */
			$imgBlock = $item->find('.product-ten__image');
			if ($imgBlock != null) {
	...
	$nextPageBlock = $page->find('.pagination__item--next');
	if ($nextPageBlock != null) {
		$nextAnchor = $nextPageBlock[0]->find('a');
		if ($nextAnchor != null) {
			$nextPage = $nextAnchor[0]->href;
			$nextPage = str_replace('/uk/', '/', $nextPage);
			sleep(1);
			parseThisPage($nextPage);
		}
	}
